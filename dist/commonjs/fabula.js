// Fabula version 0.16.0 generated by Haxe
...
fabula_Fabula.prototype = {
	...
	,getChoices: function() {
		...
		if(choices.length == 0 && this.currentSequence.autoAddChoice) {
			if(event.isExit) {
				choices.push(new fabula_Choice("EXIT",fabula_Fabula.QUIT,"quit",null,event.target,true));
			} else {
				choices.push(new fabula_Choice("CONTINUE",fabula_Fabula.CONTINUE,"continue",null,event.target));
			}
		}
		return choices;
	}
	,selectChoice: function(id,index) {
		...
		if(this.currentSequence != null && this.currentSequence.current < this.currentSequence.events.length) {
			...
			if(choice != null) {
				...
				this.completedID.push(choice.id);
				this.currentSequence.completedID.push(choice.id);
				if(this._completedCallback != null) {
...
fabula_FabulaXmlParser.parse = function(raw,fabula) {
	...
	while(_g < _g1.length) {
		...
		var events = [];
		var branches = [];
		var seq = new fabula_Sequence(seqName,kadabra_utils_XMLUtils.getBool(sequence,"autoAddChoice",true));
...
var fabula_Sequence = function(id,autoAddChoice) {
	if(autoAddChoice == null) {
		autoAddChoice = true;
	}
	this.id = id;
...
fabula_Sequence.prototype = {
	...
	,addSequence: function(events,branches) {
		if(this.events != null || this.branches != null) {
			console.log("fabula/Sequence.hx:77:","WARNING a new sequence will replace an old sequence");
		}
		this.events = events;
	...
	,getNextEvent: function(ignoreExit) {
		...
		if(!ignoreExit && this.current > -1 && this.current < this.events.length) {
			if(this.events[this.current].isExit || this.nextTarget == "$$EXIT$$") {
				this.numCompleted++;
				console.log("fabula/Sequence.hx:111:","[Fabula] sequence " + this.id + " completed");
				return null;
			}
		}
		if(this.current + 1 <= this.events.length) {
			...
			if(this.current < this.events.length) {
				...
				this.numCompleted++;
				console.log("fabula/Sequence.hx:145:","sequence completed");
			}
		}
		return null;
	}
...
fabula_condition_Variable.prototype = {
	...
	,toXMLString: function() {
		return "<variable id=\"" + this.id + "\" type=\"" + Std.string(this.type) + "\" value=\"" + Std.string(this.startingValue) + "\"/>";
	}
...
fabula_condition_VariableEnum.prototype = $extend(fabula_condition_Variable.prototype,{
	set: function(value) {
		...
		if(index == null) {
			...
			if(index >= this.options.length || index < 0) {
				console.log("fabula/condition/VariableEnum.hx:61:","[Fabula > Variable] " + index + " not in length of " + this.id);
				return false;
			}
	...
	,toXMLString: function() {
		return "<variable id=\"" + this.id + "\" type=\"" + Std.string(this.type) + "\" value=\"" + this.options.join(",") + "\"/>";
	}
	...
	set: function(value) {
		...
		if(index == null) {
			if(value == this.options.join(",")) {
				...
				console.log("fabula/condition/VariableCycle.hx:44:","[Fabula > Variable] " + value + " not in options of " + this.id);
				return false;
			}
			this.value = value;
		} else {
