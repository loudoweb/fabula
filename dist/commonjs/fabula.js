	...
	,reset: function() {
		...
		while(_g < _g1.length) {
			...
			if(seq.variables != null) {
				...
				while(_g2 < _g3) {
					var i = _g2++;
					fabula_condition_VariableCollection.getUserByIndex(seq.variables,i).reset();
				}
			}
		}
	}
...
fabula_FabulaXmlParser.parse = function(raw,conditionFactory,achievedListID) {
	...
	while(_g < _g1.length) {
		var list = _g1[_g];
		++_g;
		console.log("fabula/FabulaXmlParser.hx:71:","coucou list");
	}
...
fabula_FabulaXmlParser.parseEvents = function(sequence,seq,events,branches,parent) {
	...
	_hx_loop1: while(key.hasNext()) {
		...
		case "choice":
			if(event == null) {
				console.log("fabula/FabulaXmlParser.hx:133:","impossible to add a choice without a parent event");
				break _hx_loop1;
			}
		...
		case "event":
			...
			if(haxe_xml__$Access_HasNodeAccess.resolve(haxe_xml_Access.get_hasNode(key1),"choice")) {
				var _g = 0;
				var _g1 = haxe_xml__$Access_NodeListAccess.resolve(haxe_xml_Access.get_nodes(key1),"choice");
				while(_g < _g1.length) {
					var choice = _g1[_g];
					++_g;
...
fabula_Sequence.prototype = {
	...
	,start: function() {
		...
		if(this.variables != null) {
			...
			while(_g < _g1) {
				var i = _g++;
				fabula_condition_VariableCollection.getUserByIndex(this.variables,i).reset();
			}
		}
	}
...
fabula_condition_EVariableType.__constructs__ = [fabula_condition_EVariableType.STRING,fabula_condition_EVariableType.INT,fabula_condition_EVariableType.FLOAT];
