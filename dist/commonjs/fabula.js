...
fabula_FabulaXmlParser.parse = function(raw,conditionFactory,achievedListID) {
	...
	while(_g < _g1.length) {
		...
		if(event != null) {
			console.log("fabula/FabulaXmlParser.hx:49:",event.id);
			if(event.choices != null) {
				var _g2 = 0;
	...
	if(haxe_xml__$Access_HasNodeAccess.resolve(fabula_FabulaXmlParser._xml,"words")) {
		var word = haxe_xml__$Access_NodeListAccess.resolve(fabula_FabulaXmlParser._xml,"words")[0].elements();
		while(word.hasNext()) {
			var word1 = word.next();
			switch((word1.nodeType == Xml.Document ? "Document" : word1.get_nodeName()).toUpperCase()) {
			case "CONTINUE":
	...
	while(_g < _g1.length) {
		var list = _g1[_g];
		++_g;
		console.log("fabula/FabulaXmlParser.hx:86:","coucou list");
	}
...
fabula_FabulaXmlParser.parseEvents = function(sequence,seq,events,branches,parent) {
	...
	_hx_loop1: while(key.hasNext()) {
		...
		case "choice":
			if(event == null) {
				console.log("fabula/FabulaXmlParser.hx:148:","impossible to add a choice without a parent event");
				break _hx_loop1;
			}
		...
		case "event":
			...
			if(haxe_xml__$Access_HasNodeAccess.resolve(key1,"choice")) {
				var _g = 0;
				var _g1 = haxe_xml__$Access_NodeListAccess.resolve(key1,"choice");
				while(_g < _g1.length) {
					var choice = _g1[_g];
					++_g;
					var _choice = new fabula_Choice(kadabra_utils_XMLUtils.getString(choice,"id",fabula_FabulaXmlParser.ID_GEN_HELPER + "_C" + ++fabula_FabulaXmlParser.ID_GEN_COUNT),fabula_FabulaXmlParser.getText(choice),kadabra_utils_XMLUtils.getString(choice,"type"),fabula_FabulaXmlParser._conditionFactory.create(kadabra_utils_XMLUtils.getString(choice,"if")),kadabra_utils_XMLUtils.getString(choice,"target"),kadabra_utils_XMLUtils.getBool(choice,"exit",event.isExit));
					event.addChoice(_choice);
					if(haxe_xml__$Access_HasNodeAccess.resolve(choice,"event")) {
						fabula_FabulaXmlParser.parseEvents(choice,seq,events,branches,_choice);
					}
				}
			}
			if(parent == null) {
...
fabula_Sequence.prototype = {
	...
	,addSequence: function(events,branches) {
		if(this.events != null || this.branches != null) {
			console.log("fabula/Sequence.hx:62:","WARNING a new sequence will replace an old sequence");
		}
		this.events = events;
	...
	,getNextEvent: function(ignoreExit) {
		...
		if(!ignoreExit && this.current > -1 && this.current < this.events.length) {
			if(this.events[this.current].isExit || this.nextTarget == "$$EXIT$$") {
				this.numCompleted++;
				console.log("fabula/Sequence.hx:94:","[Fabula] sequence completed");
				return null;
			}
		}
		if(this.current + 1 <= this.events.length) {
			...
			if(this.current < this.events.length) {
				...
				this.numCompleted++;
				console.log("fabula/Sequence.hx:128:","sequence completed");
			}
		}
		return null;
	}
